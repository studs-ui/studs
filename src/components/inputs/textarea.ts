...
import style from "styles/textarea.scss?inline";
...
export class StudsTextarea extends WithForm(LitElement) {
  ...
  @property({ type: Boolean, attribute: "character-counter" }) characterCounter: TextareaProps["characterCounter"] = false;
  ...
  @property({ type: Array, attribute: "helper-text" }) helperText?: TextareaProps["helperText"] = [];
  ...
  render() {
    ...
    return html`
      <div class="textareaComponent">
        ${this.label
          ? html`<label>${this.label}${this.required ? html`<span class="required-asterisk"> *</span>` : ''}</label>`
          : ""}
        <textarea
          id="${this.textareaId}"
